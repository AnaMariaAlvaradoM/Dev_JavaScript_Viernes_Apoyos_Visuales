/* ========================================================================== 
   ANA LEARNING EXPERIENCE SYSTEM · CONTROLADOR v12 "PIZARRA" (v11 + corrección de color para Mermaid)
...
(function () {
  ...
  /* -----------------------------------------------------------------
     Regreso automático al portal
     ----------------------------------------------------------------- */
  function portalHomeUrl() {
    var currentUrl = new URL(window.location.href);
    var marker = '/docs/';
    var docsIndex = currentUrl.pathname.toLowerCase().lastIndexOf(marker);
    if (docsIndex === -1) return null;

    currentUrl.pathname = currentUrl.pathname.slice(0, docsIndex + 1) + 'index.html';
    currentUrl.search = '';
    currentUrl.hash = '';
    return currentUrl.href;
  }

  function addPortalHomeControl() {
    var controls = document.querySelector('.controls');
    var href = portalHomeUrl();
    if (!controls || !href || document.getElementById('portalHomeBtn')) return;

    var link = document.createElement('a');
    link.className = 'ctrl';
    link.id = 'portalHomeBtn';
    link.href = href;
    link.title = 'Volver al portal';
    link.setAttribute('aria-label', 'Volver al portal de clases');
    link.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 11.5 12 4l9 7.5"></path><path d="M5.5 10.5V20h13v-9.5"></path><path d="M9.5 20v-6h5v6"></path></svg>';
    controls.insertBefore(link, controls.firstChild);
  }

  addPortalHomeControl();
  ...
  /* v12: Mermaid no entiende color-mix(); se mezcla aquí y se entrega en hex. */
  function mixHex(colorA, colorB, percentA) {
    var a = colorRgb(colorA), b = colorRgb(colorB), p = percentA / 100;
    return '#' + [0, 1, 2].map(function (i) {
      var value = Math.round(a[i] * p + b[i] * (1 - p));
  ...
  function mermaidThemeVars() {
    ...
    return {
      ...
      themeVariables: {
        ...
        primaryColor: mixHex(token('--scene-accent'), token('--surf-1'), 18),
        primaryBorderColor: token('--scene-accent'),
        primaryTextColor: token('--ink'),
        secondaryColor: mixHex(token('--scene-accent-2'), token('--surf-1'), 18),
        tertiaryColor: mixHex(token('--a3'), token('--surf-1'), 18),
  ...
  try {
    ...
    console.log('Ana Learning Experience System v12 "Pizarra" · Material de autoría exclusiva.');
  } catch (error) {}
})();
